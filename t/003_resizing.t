...
$t->get_ok("/test/resized/30x30/$test_image_path")->status_is(200, "response status is 200 for /test/resized/30x30/$test_image_path")->header_is('X-Pipr-ThumbCache' => 'MISS' );
$t->get_ok("/test/resized/30x30/$test_image_path")->status_is(200, "response status is 200 for /test/resized/30x30/$test_image_path")->header_is('X-Pipr-ThumbCache' => 'HIT' );
$t->get_ok("/test/resized/30x30/$test_image_path?refresh=1")->status_is(200, "response status is 200 for /test/resized/30x30/$test_image_path")->header_is('X-Pipr-ThumbCache' => 'MISS' );
$t->get_ok("/test/resized/30x30/$test_image_path")->status_is(200, "response status is 200 for /test/resized/30x30/$test_image_path")->header_is('X-Pipr-ThumbCache' => 'HIT' );

$t->patch_ok("/test/resized/30x30/$test_image_path")->status_is(200, "response status is 200 for /test/resized/30x30/$test_image_path")->header_is('X-Pipr-ThumbCache' => 'MISS' );
$t->delete_ok("/test/resized/30x30/$test_image_path")->status_is(200, "response status is 200 for /test/resized/30x30/$test_image_path")->header_is('X-Pipr-ThumbCache' => 'MISS' );
$t->get_ok("/test/resized/30x30/$test_image_path")->status_is(200, "response status is 200 for /test/resized/30x30/$test_image_path")->header_is('X-Pipr-ThumbCache' => 'HIT' );
...
my $test_image_url_2 = '/images/test.png?lol';
my $test_image_path_2 = "public$test_image_url";
$t->get_ok($test_image_url_2)->status_is(200, 'test image exists');
...
is_deeply [imgsize(\$image)], [30,30,'WEBP'], 'Correct resized width/height (30x30)';

$image = $t->get_ok("/test/resized/100x30/$test_image_path")->tx->res->body;
is_deeply [imgsize(\$image)], [100,30,'WEBP'], 'Correct resized width/height (100x30)';

$image = $t->get_ok("/test/resized/30x/$test_image_path")->tx->res->body;
is_deeply [imgsize(\$image)], [30,24,'WEBP'], 'Correct resized width/height (30x(24))';

$image = $t->get_ok("/test/resized/x30/$test_image_path")->tx->res->body;
is_deeply [imgsize(\$image)], [38,30,'WEBP'], 'Correct resized width/height ((38)x30)';

$t->get_ok("/test/resized/30x30/$test_image_path")->header_is('Content-Type', 'image/webp');
